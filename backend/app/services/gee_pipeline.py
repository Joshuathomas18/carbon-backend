...
class GEEPipeline:
    ...
    def __init__(self):
        ...
        try:
            ...
            if settings.GEE_SERVICE_ACCOUNT_JSON and settings.GEE_PROJECT_ID:
                try:
                    credentials = self.ee.ServiceAccountCredentials(
                        "", # Extracted from JSON
                        settings.GEE_SERVICE_ACCOUNT_JSON
                    )
                    self.ee.Initialize(credentials, project=settings.GEE_PROJECT_ID)
    ...
    async def get_ndvi_history(self, lat: float, lon: float, days: int = 365) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"lat": lat, "lon": lon, "ndvi_median": val or 0.5, "data_source": "Sentinel-2"}
        except Exception as e:
    ...
    async def get_fire_history(self, lat: float, lon: float, days: int = 365) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"lat": lat, "lon": lon, "fire_detected": bool(max_fire and max_fire > 0)}
        except Exception as e:
    ...
    async def get_weather_data(self, lat: float, lon: float, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"lat": lat, "lon": lon, "total_rainfall_mm": total_rain or 0}
        except Exception as e:
